...
import { Bookmark, Clock, Heart, Star, MapPin, CalendarDays } from "lucide-react";
...
const STATS = [
  {
    ...
    value: 3,
    colorClasses: "bg-violet-50 dark:bg-violet-950 text-violet-600 dark:text-violet-400",
  },
  {
    icon: Clock,
    label: "Upcoming Stays",
    value: 1,
    colorClasses: "bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400",
  },
  {
    icon: Heart,
    label: "Saved Properties",
    value: 5,
    colorClasses: "bg-rose-50 dark:bg-rose-950 text-rose-500 dark:text-rose-400",
  },
  {
    icon: Star,
    label: "Reviews Given",
    value: 2,
    colorClasses: "bg-yellow-50 dark:bg-yellow-950 text-yellow-600 dark:text-yellow-400",
  },
];

/**
 * TEMPORARY mock booking data — UI phase only.
 * Shape mirrors the expected backend response from GET /api/bookings.
 * Will be replaced with an Axios call in the integration phase.
 */
const BOOKINGS = [
  {
    id: 1,
    propertyTitle: "Seaside Cottage in Goa",
  ...
  {
    id: 2,
    propertyTitle: "Eco Retreat in Coorg",
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          {STATS.map(({ icon: Icon, label, value, colorClasses }) => (
            ...
            >
              <div className={`w-10 h-10 flex items-center justify-center rounded-xl mb-4 ${colorClasses}`}>
                <Icon size={20} />
              </div>
        ...
        <div className="space-y-4 mb-12">
          {BOOKINGS.map(({ id, propertyTitle, location, checkIn, checkOut, status, image }) => (
            <div
              key={id}
